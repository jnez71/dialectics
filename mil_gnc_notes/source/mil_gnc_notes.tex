...
% Page layout
\geometry{top=0.99in,bottom=1in}
\hyphenpenalty = 10000
...
The $f$ is a bit gnarly, so from here on out we will assume Earth's rotation is negligible to the dynamics for simplicity. Anyway, we now see very clearly that:
\begin{enumerate}
...
$$
f(\blue{x}) = \begin{bmatrix}
(m - \rho V)\blue{R}^{\tr}\cs{E}{g}(\blue{p}) - D_p \blue{v} + \blue{R}^{\tr}\cs{E}{\mu} - m\blue \omega \times \blue v \\
-\cs{C}{r}_{\text{v}} \times \rho V \blue{R}^{\tr}\cs{E}{g}(\blue{p}) - \cs{C}{r}_{\text{d}} \times D_p \blue{v} - D_q \blue{\omega} - \blue \omega \times \cs{C}{I}_c \blue \omega
\end{bmatrix}
$$
